"""
Kalshi Fetcher using Official SDK
This replaces the custom requests-based approach with the official kalshi-python SDK
"""

from kalshi_python import Configuration, KalshiClient
from typing import List, Dict
import logging

logger = logging.getLogger(__name__)

def create_kalshi_client(key_id: str, private_key_pem: str) -> KalshiClient:
    """Create and configure Kalshi client using official SDK"""
    config = Configuration()
    config.host = "https://api.elections.kalshi.com/trade-api/v2"
    config.api_key_id = key_id
    config.private_key_pem = private_key_pem.strip()
    return KalshiClient(config)

def fetch_kalshi_markets_sdk(client: KalshiClient, limit: int = 1000) -> List[Dict]:
    """
    Fetch ALL open markets using official SDK.
    Much cleaner than custom requests - handles auth, pagination, errors automatically.
    """
    all_markets = []
    
    try:
        logger.info("Kalshi SDK: Fetching FULL market dump (all open markets)...")
        
        # SDK handles pagination automatically with cursor
        cursor = None
        page = 0
        
        while True:
            page += 1
            try:
                # Use SDK method - much cleaner!
                # Try without status filter first (SDK might handle it differently)
                if cursor:
                    response = client.get_markets(limit=limit, cursor=cursor)
                else:
                    response = client.get_markets(limit=limit)
                
                markets = response.markets
                all_markets.extend([m.to_dict() for m in markets])  # Convert to dict
                
                logger.info(f"Kalshi SDK: Page {page} - Fetched {len(markets)} markets")
                
                # Check for more pages
                if hasattr(response, 'cursor') and response.cursor and len(markets) == limit:
                    cursor = response.cursor
                else:
                    break
                    
            except Exception as e:
                logger.error(f"Kalshi SDK: Error on page {page}: {e}")
                break
        
        logger.info(f"Kalshi SDK: FULL DUMP - Fetched {len(all_markets)} total markets")
        return all_markets
        
    except Exception as e:
        logger.error(f"Kalshi SDK CRITICAL ERROR: {e}")
        return []

# Example usage:
if __name__ == "__main__":
    KALSHI_KEY_ID = "4c67f48e-3c17-43e5-8eaa-8be0bf26ac37"
    PRIVATE_KEY_BLOCK = """
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""
    
    client = create_kalshi_client(KALSHI_KEY_ID, PRIVATE_KEY_BLOCK)
    markets = fetch_kalshi_markets_sdk(client, limit=100)
    print(f"✅ Fetched {len(markets)} markets using SDK")

