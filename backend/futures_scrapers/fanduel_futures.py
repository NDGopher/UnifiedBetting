...
async def _scrape_fd_nfl_via_search() -> list[dict]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_timeout(5_000)  # let JS render

            # Bail early if PerimeterX captcha detected
            page_text_early = await page.evaluate("() => document.body ? document.body.innerText : ''")
            if "Press & Hold" in page_text_early or "confirm you are" in page_text_early:
                logger.warning("[FD] Search: PerimeterX captcha detected — skipping search page")
                dom_text = ""
            else:
                ...
                if search_input:
                    ...
                    logger.warning("[FD] Search: no search input — waiting for initial page content")
